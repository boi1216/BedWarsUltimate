...
class GameListener implements Listener
{
	...
	public function onBreak(BlockBreakEvent $event): void
	{
		...
		if (isset($this->plugin->bedSetup[$player->getName()])) {
			...
			$jsonData['teamInfo'][$setup['team']]['bedPos' . $step] = $block->getPosition()->getX() . ":" . $block->getPosition()->getY() . ":" . $block->getPosition()->getZ();
		...
		if ($playerGame !== null) {
			if ($playerGame->getState() == Game::STATE_LOBBY) {
				...
				foreach ($playerGame->teamInfo as $team => $data) {
					...
					if ($spawn->distance($block->getPosition()) < 6) {
						$event->cancel();
					} else {
						$playerGame->placedBlocks[] = Utils::vectorToString(":", $block->getPosition());
					}
				}
