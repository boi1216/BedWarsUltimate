				...
				foreach ($playerTeam->getPlayers() as $player) {
					foreach ($player->getInventory()->getContents() as $index => $item) {
						if ($item instanceof Sword) {
							$item->addEnchantment(new EnchantmentInstance(EnchantmentIdMap::getInstance()->fromId(EnchantmentIds::SHARPNESS)), $playerTeam->getUpgrade('sharpenedSwords'));
			...
			case 'armorProtection';
				...
				foreach ($playerTeam->getPlayers() as $player) {
					...
					foreach ([$helmet, $chestplate, $leggings, $boots] as $armor) {
						$armor->addEnchantment(new EnchantmentInstance(EnchantmentIdMap::getInstance()->fromId(EnchantmentIds::PROTECTION)), $playerTeam->getUpgrade('armorProtection'));
					}
